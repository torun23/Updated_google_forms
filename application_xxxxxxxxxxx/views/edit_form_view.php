...
<?php endforeach; ?>
    ...
    $(document).ready(function () {
        ...
        function appendNewOption(section, questionType, options = []) {
            var iconClass = "";
            ...
            var optionHtml = options.map(option => `
                <div class="option">
                    <i class="fas ${iconClass} icon-transparent"></i>
                    <input type="text" class="form-control option-label" placeholder="Option" value="${option}">
                    <span class="delete-option-icon">&times;</span>
                </div>
            `).join('');
        ...
        function createFormSection() {
            var sectionHtml = `
                <div class="form-section" data-options="">
                    <div class="header-row">
            ...
            newSection.find('.question-type').on('change', function() {
                ...
                var storedOptions = newSection.data('options') || [];
                ...
                if (selectedType === 'multiple-choice' || selectedType === 'checkboxes' || selectedType === 'dropdown') {
                    appendNewOption(newSection, selectedType, storedOptions);
                ...
                var currentOptions = newSection.find('.option-label').map(function() {
                    return $(this).val();
                }).get();
                newSection.data('options', currentOptions);
            });
        ...
        $('.question-type').on('change', function() {
            ...
            var storedOptions = section.data('options') || [];
            
            optionsContainer.empty(); // Clear previous options

            if (selectedType === 'multiple-choice' || selectedType === 'checkboxes' || selectedType === 'dropdown') {
                appendNewOption(section, selectedType, storedOptions);
            ...
            // Backup the current options
            var currentOptions = section.find('.option-label').map(function() {
                return $(this).val();
            }).get();
            section.data('options', currentOptions);
        });
